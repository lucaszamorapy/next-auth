...
import { formSchema, AuthFormData } from "@/app/(auth)/login/validators";
...
export function LoginForm({
  ...
  const handleSubmit = async (social: "github" | "google") => {
    try {
      await doSocialLogin(social);
  ...
  const onSubmit = async (data: AuthFormData) => {
    try {
      const res = await doCredentialsLogin(data);
      if (res.code !== 200) {
    ...
    <div className={cn("flex flex-col gap-6", className)} {...props}>
      <Card>
        <CardHeader className="text-center">
          ...
          <CardDescription>
            Entre com a sua conta do Github ou Google
          </CardDescription>
        </CardHeader>
